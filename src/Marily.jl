...
function run_server_threaded(port::Int, handler::Function;
    ...
    # Create a channel for distributing events to worker threads
    event_channel = Channel{Any}(100)  # Buffer up to 100 events
    ...
    main_thread = Threads.@spawn :interactive try
        while true
            ...
            for event in events
                put!(event_channel, event)
            end

            # Sleep if requested and we didn't get any events
            if isempty(events) && polling_interval > 0
